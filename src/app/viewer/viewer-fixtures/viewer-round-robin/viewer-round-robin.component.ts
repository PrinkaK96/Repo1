...
@Component({
  selector: 'stupa-viewer-round-robin',
  templateUrl: './viewer-round-robin.component.html',
  styleUrls: ['./viewer-round-robin.component.scss'],
  providers: [MessageService, ConfirmationDialogService],

})
export class ViewerRoundRobinComponent implements OnInit{
 ...
 constructor(private eventsService: EventsService, private confirmationDialogService: ConfirmationDialogService, private azureLoggerService: MyMonitoringService,
 ...
 //#region method is used for open dialog and get group matches base on categoryId for schedule on button click
 openDialog() {
   this.getGroupMatchsForSchedule(this._currentCategoryId);
   this._openSchedule = true;
 }
 //#endregion method is used for open dialog and get group matches base on categoryId for schedule on button click
 //#region method is used for get group matches for base on category_id for schedule popup
 getGroupMatchsForSchedule(category_id: any) {
   this._matchDetailsList = [];
   this._matchDetailsListCopy = [];
   this._showLoader = true
   this.eventsService.getGroupMatchDetailsV3(this._eventId, category_id, 1)
     .subscribe({
       next: (result: any) => {
         this._showLoader = false
         this._matchDetailsList = [];
 ...
 //#endregion method is used for get group matches for base on category_id for schedule popup
 ...
 //#endregion used for Check Matches finished for particul category
 // getListWithCategoryId(categoryId: any) {
 //   this.getGroupMatchsForSchedule(categoryId);
 // }
