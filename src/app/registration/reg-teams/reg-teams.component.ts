...
export class RegTeamsComponent {
  ...
  constructor(private dashboardService: DashboardService, private eventService: EventsService, private azureLoggerService: MyMonitoringService,
    private messageService: MessageService, private profileLettersService: ProfileLettersService,
  ...
  currentEvent(eventDetails: any, index: any, catIndex: any) {
    if (eventDetails.target.checked) {
      const data = {
        ...
        "player_id": this._players[index].player_id,
  ...
  //#endregion
  //#region This method is used for Get List of teams
  getTeams() {
  ...
  //#endregion
  //#region This method is used for payment
  payNow() {
  ...
  //#endregion
  //#region This method is used Get Status of payment
      ...
      for (let k = 0; k < this._selectedPlayerList.length; k++) {
        if (this._selectedPlayerList[k].cat_Id == this._selectedCategoryList[i]) {
          ...
          list =
          {
            ...
            "ref_id": this._selectedPlayerList[k].player_id,
            "players": this._teamPlayerList,
            "rating": this._selectedPlayerList[k].rating
          }
          // participants.push(list);
          if (this._participantList.findIndex((x: any) => x.ref_id == this._selectedPlayerList[k].player_id) == -1) {
