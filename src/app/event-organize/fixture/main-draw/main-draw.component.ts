...
export class MainDrawComponent implements OnInit {
  ...
  getKnockoutTournament() {
    ...
    if (
      ...
      this.fixtureService
        .getKnockoutTournament(this._eventId, this._currentCategoryId)
        .subscribe({
          next: (result: any) => {

            this.getPlayerList();
            if (result.body.length > 0) {
              if (this.singleEliminationTournament == undefined) {
                this.singleEliminationTournament = {
                  rounds: [
                  ],
                };
      ...
      this.confirmationDialogService
        ...
        .then((confirmed) => {
          if (confirmed) {
            ...
            this.fixtureService
              .deleteKnockout(this._eventId, this._currentCategoryId, 2)
              .subscribe({
                next: (result: any) => {
